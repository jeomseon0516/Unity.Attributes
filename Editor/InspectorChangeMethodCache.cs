...
namespace Jeomseon.Attribute.Editor
{
    ...
    /// </summary>
    internal static class InspectorChangeMethodCache
    {
        private static readonly Dictionary<
            Type,
            IReadOnlyDictionary<
                string,
                EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>[]>> Cache =
            new();

        public static IReadOnlyDictionary<
            string,
            EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>[]> Get(
            Type targetType)
        {
            if (targetType == null)
            ...
            if (Cache.TryGetValue(
                    ...
                    out IReadOnlyDictionary<
                        string,
                        EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>[]> methods))
            {
                return methods;
            }
        ...
        private static IReadOnlyDictionary<
            string,
            EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>[]> BuildIndex(
            Type targetType)
        {
            Dictionary<
                string,
                List<EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>>> collecting =
                new(StringComparer.Ordinal);

            IEditorMethodTriggerHandler handler =
                EditorMethodTriggerHandlerRegistry
                    .Get<InvokeOnInspectorChangeAttribute>();

            foreach (EditorTriggeredMethod<InvokeOnInspectorChangeAttribute> item in
                     EditorTriggeredMethodCache<InvokeOnInspectorChangeAttribute>.Get(
                         targetType))
            {
                        ...
                        out string reason))
                {
                     ...
                     * 있습니다. InvokeOnInspectorChange에서 매개변수를 지원할 때는 변경 대상,
                     ...
                     */
                    Debug.LogWarning(
                        $"[{nameof(InvokeOnInspectorChangeProcessor)}] " +
                        $"지원하지 않는 메서드입니다: " +
            ...
            Dictionary<
                string,
                EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>[]> result =
                new(collecting.Count, StringComparer.Ordinal);

            foreach (KeyValuePair<
                         string,
                         List<EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>>>
                     pair in collecting)
            {
        ...
        private static void AddMethod(
            IDictionary<
                ...
                methodsByField,
            string fieldName,
            EditorTriggeredMethod<InvokeOnInspectorChangeAttribute> method)
        {
            if (!methodsByField.TryGetValue(
                    fieldName,
                    out List<EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>>
                        methods))
            {
                methods =
                    new List<EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>>();
        ...
        private static bool ContainsMethod(
            IEnumerable<EditorTriggeredMethod<InvokeOnInspectorChangeAttribute>> methods,
